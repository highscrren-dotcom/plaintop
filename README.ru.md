...
| **`player/`** | виджет «сейчас играет»: трек, полоса позиции и кнопки текстом, читает MPRIS через модуль медиаконтроллера Plasma | работает |
...
показан тот, кто играет. Настройка «Мышь» работает как у двух других, с одним следствием:
пока клики проходят насквозь, кнопки нажать нельзя — поэтому по умолчанию она выключена
(решение 11). Ставится `./install.sh --player`.
...
обрыв сети — с пометкой об обрыве, а через час и с возрастом. Место берётся из поиска на
