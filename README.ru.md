...
⚠️ Оба виджета перехватывают мышь, поэтому у стола под ними нет контекстного меню и рамки
выделения. Это не недоделка: плазмоид на рабочем столе клики отдать не может — три способа
проверены и провалились. Подробности и то, что всё-таки работает (обычное окно с
`Qt.WindowTransparentForInput`), — в [docs/GOTCHAS.ru.md](docs/GOTCHAS.ru.md).
...
**Про мышь.** Оба виджета перехватывают клики, и обойти это нечем: обёртка апплета в
оболочке слушает нажатие сама. Настройка «Мышь» и стоящие за ней ключи
`./install.sh --clicks-on` / `--clicks-off` снимают приём ввода только у самого
представления виджета — этого недостаточно, и в `docs/GOTCHAS.ru.md` записано, что
пробовали. Оставлены потому, что ничего не стоят и были бы первой половиной настоящего
решения.
