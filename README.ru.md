...
| Каталог | Что это |
|---|---|
| **`plasmoid/`** | целевая реализация: виджет Plasma 6 на QML. Здесь идёт работа |
| **`conky/`** | первая реализация на [conky](https://github.com/brndnmtthws/conky). Работает, остаётся до полной замены плазмоидом |
| **`spectrum/`** | второй виджет: визуализатор звука в том же плоском стиле, со своей службой-реле |
...
**Клики по умолчанию проходят насквозь.** Плазмоид на рабочем столе мышь перехватывает,
и стол под ним теряет контекстное меню и рамку выделения — поэтому у обоих виджетов есть
настройка «Мышь», снимающая приём ввода. Путь обратно идёт не через диалог самого виджета:
когда клики проходят, по нему нечем щёлкнуть правой кнопкой.

```bash
./install.sh --clicks-off   # виджеты снова ловят мышь, их можно настраивать
./install.sh --clicks-on    # клики уходят на рабочий стол
```
