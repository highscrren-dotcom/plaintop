...
if ! command -v docker >/dev/null; then
    :
elif states=$(docker ps -a --format '{{.State}}' 2>/dev/null); then
    printf 'docker|%s|%s\n' "$(grep -cE '^(running|paused|restarting)$' <<<"$states")" \
        "$(grep -c . <<<"$states")"
else
    printf 'docker|noaccess\n'
fi
...
# Updates: from the LOCAL database (pacman -Qu), without a network sync. Not an Arch
# system — no line at all rather than a false "0".
...
if [ -z "$upd" ]; then
    upd=$(LC_ALL=C pacman -Qu 2>/dev/null | grep -vc '\[ignored\]')
