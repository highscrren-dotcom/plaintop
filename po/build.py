...
window hosts' setup.py, which deploys the same files into ~/.local/share/locale. The
output directory is emptied first, so a language dropped from po/ does not linger in a
package. Only msgfmt is needed here; refreshing the catalogs is extract.py's job.
