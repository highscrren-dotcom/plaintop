...
## Плазмоид на рабочем столе не умеет отдавать клики столу

Три попытки, все исполнены, все мимо:

| Попытка | Итог |
|---|---|
| `enabled: false` у полного представления | клик до стола всё равно не доходит |
| `locked = true` через скриптинг plasmashell | в Plasma 6 свойство только читается, остаётся `false` |
| `immutability=2` у контейнера, записанное при остановленной оболочке | применилось и устояло, клики по-прежнему перехватываются |

Причина в коде самой оболочки: `BasicAppletContainer` оборачивает каждый апплет, и его
основа на C++ слушает нажатие сама — ей нужно долгое нажатие для входа в режим правки
(`editModeCondition: AfterPressAndHold`). Из QML апплета от этого не отказаться.
